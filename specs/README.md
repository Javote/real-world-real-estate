...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | abierta 🟡 — **§A (6 rutas) cerrada 2026-09-20**: migrada a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-notary.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de `notary`. Quedan §B, §C, §D |
