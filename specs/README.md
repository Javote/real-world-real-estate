...
| [`SPEC-216`](SPEC-216-orpc-en-los-11-routers-restantes.md) | Extender oRPC a los routers cross-cutting/admin: 38 rutas mecánicas | anexo (nace de `SPEC-212`) | **desarrollada 2026-09-20 — sin implementar.** Auditadas las 39 rutas de los 11 routers una por una; 38 repiten patrones ya probados en `SPEC-212` §A-§D (`scopeEnQuery`, `successStatus`, `.errors()`, prefijo compartido) sin nada nuevo — 7 sub-partes sugeridas (§E1-§E7). La única excepción, `GET /evidence/:id/download`, se separó a `SPEC-217` |
