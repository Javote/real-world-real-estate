...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | abierta 🟡 — **§A, §B y §C cerradas 2026-09-20** (las 26 rutas de las tres): migradas a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-{notary,certifier,investor}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de ninguna de las tres. `GET .../dossier/export.pdf` (PDF binario) migró devolviendo un `File` en vez de un `Buffer` — el adaptador Node de oRPC manda los bytes crudos cuando el body es `instanceof Blob`. Queda §D |
