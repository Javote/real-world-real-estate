...
| 2 | Unit tests **≥95% coverage** | Cobertura de líneas ≥95% en **toda la app** (relectura del dueño, 2026-09-21: separar una parte es artificial) | ⚠️ **Falta la web: [`SPEC-019`](SPEC-019-cobertura-de-apps-web.md), abierta** (37% de líneas). [`SPEC-017`](SPEC-017-cobertura-95-en-toda-la-app.md) cerró shared, cardano y contratos, y dejó la API en 97,45% de líneas; la vara más estricta de la API (branches ≥95%) sigue en [`SPEC-018`](SPEC-018-cobertura-de-apps-api.md). Antes esta fila daba ✅ leyendo el criterio solo sobre los contratos |
...
| [`SPEC-018`](SPEC-018-cobertura-de-apps-api.md) | Cobertura de `apps/api`: branches ≥95%, el resto ≥98% | **abierta 2026-09-22** — vara más estricta que el criterio 2, pedida por el dueño. Paso 0 cerrado el mismo día (helper `delegarAOrpc` + 40 ramas inalcanzables marcadas en su línea): branches 81,8% → 91,3%. **Analizadas rama por rama las 75 restantes**: 57 alcanzables con receta, 18 a marcar, y 2 bugs de 500. Quedan seis lotes paralelizables |
| [`SPEC-019`](SPEC-019-cobertura-de-apps-web.md) | Cobertura de `apps/web`: las cuatro métricas ≥95%, más CI y evidencia | **abierta 2026-09-22** — lo único que separa al criterio 2 de ✅. Web en 37% de líneas. **Analizadas las 1.387 branches sin cubrir, archivo por archivo**: 29 no se alcanzan (casi todas se borran en W0), un bug, y una receta por archivo. Un paso 0 serial y nueve lotes paralelizables |
...
| [`SPEC-112`](SPEC-112-pasada-de-accesibilidad-con-voiceover.md) | Accesibilidad: las tres capas automatizadas (Biome `a11y`, Vitest+`axe-core`, Playwright+`@axe-core/playwright`) + la pasada manual con VoiceOver | anexo (nace de separar `SPEC-104`) | **abierta 2026-09-22** — sin fecha, nivel 🟢 independiente. `vitest-axe` se probó y se descartó (su `dist/extend-expect.js` publicado está vacío); se define `axe-core` crudo con un matcher propio, verificado en vivo |
