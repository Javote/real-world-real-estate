...
| 5 | Endpoints documentados | OpenAPI — sale solo del contrato oRPC (D-066) | ✅ [`specs/postman/propnexus.postman_collection.json`](postman/propnexus.postman_collection.json) — 88 rutas, generado desde el router montado (`pnpm --filter @plataforma/api docs:api`), no mantenido a mano; `test/api-docs-freshness.test.ts` lo mantiene sincronizado. Los 7 endpoints del camino feliz (login, crear proyecto, crear stage, subir evidencia, aceptar invitación, transicionar stage, liberar pago) traen body de ejemplo real; el resto de las 88 queda con método+path+auth |
