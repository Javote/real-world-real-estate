...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | **cerrada 2026-09-20 — las cuatro sub-partes** (45 de las 46 rutas de D-066): migradas a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-{notary,certifier,investor,developer}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de las 45. `GET .../dossier/export.pdf` (§C) migró devolviendo un `File` en vez de un `Buffer` — el adaptador Node de oRPC manda los bytes crudos cuando el body es `instanceof Blob`. **§D dejó una ruta afuera a propósito**: la subida multipart de `developer-evidencia.routes.ts` sigue con Multer — `OpenAPIHandler` sabe parsear `multipart/form-data`, pero bufferea el archivo entero en memoria sin límite configurable, y migrarla habría empeorado la deuda de RAM que `CLAUDE.md` raíz ya declara. **Dos investigaciones quedan abiertas, pedidas por el dueño 2026-09-20** (el código de las 45 rutas está cerrado, esto no lo bloquea): la del gancho `interceptors` para que un error no clasificado llegue a Sentry ya tiene camino técnico confirmado (un interceptor con `try/catch` alrededor de `next()` ve la excepción cruda antes del 500 genérico — probado con smoke test) pero falta decidir el punto único de construcción, no implementado; y la de `Multer + call()` para la única ruta multipart que quedó afuera también está resuelta como investigación — con un hallazgo lateral no buscado: **las 45 rutas ya cerradas de §A-D no devuelven `error.flatten()` en un 400 de validación, devuelven el shape de `ORPCError.toJSON()`** (`{code, status, data: {issues}}`), verificado contra una ruta real (`POST /notary/dossiers/:id/reject`) — ningún test lo fijaba, así que nadie lo había notado. Ninguna de las dos investigaciones está implementada — ver la spec §Pendiente ×2 |
