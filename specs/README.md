...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | **cerrada 2026-09-20 — las cuatro sub-partes** (45 de las 46 rutas de D-066): migradas a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-{notary,certifier,investor,developer}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de las 45. `GET .../dossier/export.pdf` (§C) migró devolviendo un `File` en vez de un `Buffer` — el adaptador Node de oRPC manda los bytes crudos cuando el body es `instanceof Blob`. **§D dejó el PARSEO del multipart afuera a propósito**: la subida de `developer-evidencia.routes.ts` sigue con Multer — `OpenAPIHandler` sabe parsear `multipart/form-data`, pero bufferea el archivo entero en memoria sin límite configurable, y migrar el parseo habría empeorado la deuda de RAM que `CLAUDE.md` raíz ya declara. **Investigación del gancho `interceptors` para Sentry, pedida por el dueño 2026-09-20, queda abierta** (el código de las 45 rutas está cerrado, esto no lo bloquea): camino técnico confirmado (un interceptor con `try/catch` alrededor de `next()` ve la excepción cruda antes del 500 genérico — probado con smoke test) pero falta decidir el punto único de construcción, no implementado. **La segunda investigación (`Multer + call()`), en cambio, se implementó el mismo día, en alcance acotado**: `POST /developer/projects/:id/stages/:stageId/evidence` sigue con Multer parseando el multipart, pero la validación de los campos de texto pasó de `safeParse` a `call()` de oRPC — el 400 de esta ruta ya tiene el mismo shape (`ORPCError.toJSON()`, `{code, status, data: {issues}}`) que las otras 45, cerrando la divergencia que la investigación encontró (las 45 rutas de §A-D nunca devolvieron `error.flatten()`, solo esta lo hacía). El resto del handler (storage, bundle, anclaje, notificaciones, audit log, transición de stage) no se tocó — sigue siendo Express/Kysely plano. Ver la spec §Cerrado y §Pendiente |
...
| [`SPEC-216`](SPEC-216-orpc-en-los-11-routers-restantes.md) | Extender oRPC a los routers cross-cutting/admin: 38 rutas mecánicas | anexo (nace de `SPEC-212`) | **§E1, §E2, §E3 y §E5 cerradas 2026-09-20 (14 de las 38 rutas), quedan §E4 (🔴 bcrypt, aislada a propósito), §E6 y §E7.** §E1 (`profile`+`notifications`, 5 rutas) y §E2 (`auth`+`public`, 3 rutas) en un commit; §E3 (`audit`+`contracts`, 3 rutas) y §E5 (`stages`, 3 rutas) en otro — combinadas porque ninguna toca superficie 🔴, sin romper el aislamiento que la spec pide para §E4. Mismo patrón que `SPEC-212` §A-§D en las cuatro: cliente tipado probado contra el servidor real (`test/orpc-client-{profile,notifications,auth,public,audit,contracts,stages}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin las 14 entradas. `auth.routes.ts` es el primer router con un contexto mixto (`AuthContext` con `user` opcional) — tipa con un solo `$context`. `stages.routes.ts` concentra cinco `.errors()` con nombre (`STAGE_IDENTITY_IMMUTABLE`, `STAGE_TRANSITION_FORBIDDEN`, `STAGE_TRANSITION_INVALID`, `STAGE_EVIDENCE_REQUIRED`, `STAGE_EVIDENCE_UNATTRIBUTED`) y `contracts.routes.ts` es la primera vez que se migra una ruta con la regla disyuntiva `{ alguna: [...] }` delante — `authorize` la resuelve en Express antes de que oRPC vea la request, sin sorpresas. Dos tests de `auth.test.ts` se actualizaron para el nuevo shape de error (`ORPCError.toJSON()`, no `error.flatten()`) — el mismo cambio que ya habían aceptado las 45 rutas de `SPEC-212`. Auditadas las 39 rutas de los 11 routers una por una; 38 repiten patrones ya probados en `SPEC-212` §A-§D sin nada nuevo. La única excepción, `GET /evidence/:id/download`, se separó a `SPEC-217` |
| [`SPEC-217`](SPEC-217-el-streaming-de-download-necesita-su-propia-investigacion.md) | `GET /evidence/:id/download`: streaming real con oRPC | anexo (nace de `SPEC-216`) | **investigada y verificada 2026-09-20 — sin implementar.** Un `ReadableStream` web como output, con `outputStructure: "detailed"`, pasa sin bufferear por las tres capas de oRPC (codec → adaptador Node → `sendStandardResponse`) — confirmado leyendo el código fuente de las tres y con un smoke test de 5MB, bytes idénticos. De paso cierra un defecto latente: el `.pipe(res)` de hoy no maneja `'error'` a mitad de stream, el de oRPC sí |
