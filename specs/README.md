...
| 11 | **Sin hallazgos P1** de seguridad | Reporte de security review | ✅ [`evidencia-m3/4-seguridad/SECURITY-REVIEW-2026-09.md`](evidencia-m3/4-seguridad/SECURITY-REVIEW-2026-09.md) — 3 P1 cerrados, análisis estático (Semgrep) en CI, `pnpm audit` en CI en cada corrida (job `app`, gate en crítico + reporte completo no bloqueante), 0 P1 abiertos |
...
**Tres estados.** **Cerrada**: hecha y verificada. **Postergada**: decidida a propósito, con su razón escrita en la fila, y todavía no hecha o no del todo verificada — no es trabajo olvidado ni pendiente de la próxima tanda. **Abierta**: lista para tomarse ahora. Hoy **no hay ninguna abierta** y hay **seis postergadas**: `304`, `305`, `402`, `407` y `408` esperan a mainnet (`CLAUDE.md` raíz §Antes de mainnet, después del Milestone 3); `104` espera una pasada manual con VoiceOver que no es código. `219` se cerró el 2026-09-21: el Paso 0 midió cero duplicados en producción y no hizo falta postergar nada.
...
| [`SPEC-104`](SPEC-104-live-regions.md) | Anunciar lo que cambia: las live regions que la app no tiene | F-03 | **postergada** · pasada manual con VoiceOver — el código y los tests automatizados están hechos (cerrada 2026-09-19); lo que falta es la pasada que pide la spec, y no es código |
