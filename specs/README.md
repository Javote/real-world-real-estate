...
| [`SPEC-216`](SPEC-216-orpc-en-los-11-routers-restantes.md) | Extender oRPC a los routers cross-cutting/admin: 38 rutas mecánicas | anexo (nace de `SPEC-212`) | **§E1 y §E2 cerradas 2026-09-20 (8 de las 38 rutas), el resto (§E3-§E7) sin implementar.** §E1 (`profile`, `notifications`, 5 rutas) y §E2 (`auth`, `public`, 3 rutas) migradas a oRPC, mismo patrón que `SPEC-212` §A-§D — cliente tipado probado contra el servidor real (`test/orpc-client-{profile,notifications,auth,public}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin las 8 entradas. `auth.routes.ts` es el primer router con un contexto mixto (`AuthContext` con `user` opcional: `POST /login` nunca lo tiene, `GET /me` siempre) — tipa con un solo `$context`, no hicieron falta dos routers de generación separados. Dos tests de `auth.test.ts` se actualizaron para el nuevo shape de error (`ORPCError.toJSON()`, no `error.flatten()`) — el mismo cambio que ya habían aceptado las 45 rutas de `SPEC-212`. Auditadas las 39 rutas de los 11 routers una por una; 38 repiten patrones ya probados en `SPEC-212` §A-§D (`scopeEnQuery`, `successStatus`, `.errors()`, prefijo compartido) sin nada nuevo. La única excepción, `GET /evidence/:id/download`, se separó a `SPEC-217` |
