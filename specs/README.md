...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | abierta 🟡 — **§A y §B cerradas 2026-09-20** (12 rutas): las dos migradas a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-notary.test.ts`, `test/orpc-client-certifier.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de ninguna de las dos. Quedan §C, §D |
