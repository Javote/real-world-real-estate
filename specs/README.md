...
| [`SPEC-212`](SPEC-212-contrato-en-la-firma-de-la-ruta.md) | El contrato en la firma de la ruta, con oRPC (D-066) — §A notary, §B certifier, §C investor, §D developer | B-06 | abierta 🟡 — **§A, §B y §C cerradas 2026-09-20** (25 rutas de 26): las tres migradas a oRPC 1.15.2, cliente tipado probado end-to-end contra el servidor real (`test/orpc-client-{notary,certifier,investor}.test.ts`), `REQUEST_SCHEMAS`/`RESPONSE_SCHEMAS` sin entradas de ninguna de las tres. Una ruta de §C (`GET .../dossier/export.pdf`) queda deliberadamente sin migrar: devuelve un PDF binario, y `OpenAPIHandler` solo serializa JSON. Queda §D |
