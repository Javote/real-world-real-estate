...
| 14 | Semgrep (`gha-curl-pipe-shell`) marcaba el login de `.github/workflows/reconcile.yml`: `curl … \| python3 -c`. El pipe llevaba **datos** (JSON) a un script literal, no código bajado de la red, pero la forma es la del patrón `curl \| sh` | Bajo (falso positivo de forma) | Cerrado | 2026-09-21 | la respuesta se guarda en una variable y se parsea aparte; un `nosemgrep` no servía porque el hallazgo se reporta sobre el bloque `run:` entero. Verificado con Semgrep 1.176.1 y los mismos rulesets de CI: 1 hallazgo antes, 0 después |
