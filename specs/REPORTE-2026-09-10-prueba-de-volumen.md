...
- **Etapa 3 — corrección de datos, no reintento.** El hilo on-chain real de esta etapa vive en
  `9a57f563e7d5627f22d9a062d26049d48789c91014d8b676298c0eb7c71f9e68#0`, en estado `InProgress`.
  Reintentar "Reanudar etapa" sin corregir el `outputRef` registrado fallaría de nuevo (seguiría
  apuntando al UTxO ya gastado). Esto es una corrección de datos sobre un registro on-chain real
  — nivel 🟡/🔴 según se mire (toca directamente qué UTxO la aplicación cree que es la fuente de
  verdad de un hilo, D-058) y **no se hizo en esta sesión**: queda para que el dueño decida cómo
  reconciliar (¿una ruta nueva que resuelva el UTxO real por `stageId` contra la cadena y corrija
  el `outputRef`? ¿corrección manual puntual?).
- **Etapa 4 — reintento simple, una vez corregida la causa concurrente.** Su UTxO nunca se tocó,
  así que un nuevo intento limpio de "Reanudar etapa" (sin superponerse con otra acción de anclaje)
  debería comportarse como cualquiera de las 28 exitosas.
- `retry-anchor` (la ruta que ya existe) **no cubre ninguno de los dos casos** tal cual: está
  pensada para "un mint que falló de verdad sobre un stage que sigue en `Pending`", no para una
  transición intermedia con UTxO desincronizado (etapa 3) ni para un simple reintento de
  transición (etapa 4, que ni siquiera tiene un endpoint de reintento — solo se puede volver a
  invocar el mismo mecanismo de "Reanudar etapa"/"Certificar" desde cero).

## Qué falta después de este reporte

1. **Decidir qué hacer con el hilo huérfano de la etapa 3** (Torre Volumen 1 · "Movimiento de
   suelos y excavación"): su UTxO real y vigente es
   `9a57f563e7d5627f22d9a062d26049d48789c91014d8b676298c0eb7c71f9e68#0`, `InProgress`, pero la
   aplicación no lo sabe. Es una corrección de datos sobre un registro on-chain real (🟡/🔴, toca
   D-058) — requiere decisión del dueño sobre el mecanismo (¿ruta de reconciliación por `stageId`
   contra la cadena? ¿corrección manual puntual?), no se resolvió en esta sesión.
2. **Reintentar "Reanudar etapa" sobre la etapa 4** (Cimentación) una vez descartada — o corregida
   — la causa de la posible colisión concurrente con la etapa 3; su UTxO nunca se tocó, así que un
   reintento limpio debería comportarse como cualquiera de las 28 exitosas.
3. ~~Confirmar con logs de servidor la causa~~ **Hecho.** Ver §La causa arriba — confirmado por
...
4. **Correr una pasada de reconciliación** sobre los TXIDs con estado `Pending` en la base antes
   de publicar la lista formal (3.10), para que el campo `status` refleje la confirmación real en
   Preprod y no dependa de que alguien abra la pantalla correcta.
5. **Verificar el resto del apéndice contra el explorador.** Esta sesión confirmó contra Preprod
   (vía Koios, `preprod.koios.rest`, sin necesitar `BLOCKFROST_API_KEY`) un puñado de TXIDs
   puntuales — los tres del hallazgo (`fc37cac7…`, `270cfea4…`, `9a57f563e…`) y uno de control sano
   (`a9cf646a…`) — que resultaron ser reales y consistentes con lo que dice la base. No se
   verificaron los 176 uno por uno; dado que Koios no requiere credenciales y no tuvo el bloqueo de
   Cloudflare que sí afecta a `preprod.cardanoscan.io` en el navegador, es el camino recomendado
   para una verificación masiva antes de publicar 3.10.
6. **Medir el balance real de la wallet de servicio** antes/después para contrastar contra el
   presupuesto estimado de ~35 ADA × 3 proyectos ≈ 105 ADA — no se volvió a medir en esta sesión.
7. Con esto cerrado, quedan habilitados **3.10** (lista formal de TXIDs — este reporte es la base),
   **3.12** (video walkthrough, grabable sobre estos mismos datos) y **3.13** (recontacto a
   pilotos), en ese orden, según el plan.
...
## Apéndice — TXIDs por etapa (los 176 reales)

Agrupados por proyecto y etapa, en el orden de la FSM. `(sin TXID — ver hallazgo)` marca los 4
casos de Torre Volumen 1 / etapas 3 y 4 descritos arriba. El estado en base sigue la nota de
§Resultados agregados — `Pending` no implica "no confirmado en Preprod", implica "sin
reconciliación de lectura todavía".
...
| Ancla evidencia | Pending | `bce9a2ac69130202ffdc6cd580b6f63fb102093d92eec2e095883172d0722485` |
...
| Ancla evidencia | Pending | `b377f2b157b47f9a98d02c418f7423dc08613dd62bda6b03cfd1b26cb37e9ece` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `61067ba2fe920bfbb350559ba7007a7beed7dd9790713878df3fd0ba3b3b6240` |
| Ancla evidencia | Pending | `0132d1497f2e63247bd78d13728c46c31748114bb3de7e838535c9e428eafb2c` |
...
| Ancla evidencia | Pending | `46bf2b8c64b10eb26a99005d888339c423ae692eee5d3b02fddfe9d5e40737e7` |
...
| Ancla evidencia | Pending | `113e729ec564b8f3c748145bd0645a2b56fbfc54654202d1e20f116f843addfc` |
...
| Certificar (InProgress→Completed) | Pending | `636260b0b67c0fc9f56a5baef91864c82221bf2dde0f653dba119ca0d3ba56f6` |
...
| Ancla evidencia | Pending | `b6e9e7d651d15a69d7757a039ec2dc56ba4b82352a300ae545aa09b6deb4b6ad` |
...
| Reanudar (Observed→InProgress) | Pending | `d5e8f54c298f8fb0f42a16685090526af1466a55e5a5ed733e065c47cecc1556` |
...
| Ancla evidencia | Pending | `4aa94e5207574700a1adc214ac9551c947fc76151e248eea02b4d4e57263411b` |
| Auto Pending→InProgress | Confirmed | `17b8ba858673a85930a20c98f25a4cf9ab0ac11715b6924b3e22b9c4e82c015f` |
| Observar (InProgress→Observed) | Pending | `fac73f94fad34601f9cbdc2ad8b0b4ad8bff371248430c762dcde6ff2f0414c2` |
...
| Ancla evidencia | Pending | `f73af36678a0f54ee50ce54dec25ab9899c2a018c14c4dc6659e5f0905a13ab8` |
| Auto Pending→InProgress | Confirmed | `fd78664b3b6599ff99097f1fd2e7bd99c70cf9afbecd4fe2d1bbc071a8b853fd` |
| Observar (InProgress→Observed) | Pending | `cb46248e9fed481262dda1d514e05b26db8dbf337e747dc5203c397794eabdbb` |
| Reanudar (Observed→InProgress) | Confirmed | `dcef912606d674522fb6a581e14d48a457683b1ea77898646486f1138a6615b6` |
| Certificar (InProgress→Completed) | Pending | `ead1aa198a77e19bb2d73bc3529203e08afe8b50aae334edf66b766253f5dfca` |

**Etapa 9 — Terminaciones**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `085ebd2ec2b589ac2baf8dbae106118c74e27d24a02eb9811078f0b5cfe2c6fb` |
| Ancla evidencia | Pending | `6c6c1e208285c9395a099d0f5502e3d9ab5083adbc8629068decf9241254012d` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `fe3d21b358bf931e5f78dbc559f66cc2b2b341a9c5a62dc95c504dd6ef0af2b2` |
| Ancla evidencia | Pending | `36fce09bd1a041b41fbb04fa573b1f4f253e95e0cd62d0bc2cf9dd010e346dad` |
...
| Certificar (InProgress→Completed) | Pending | `ab9794a5010eb0d6f672cd0e9bc45f1499a8e38f904e3ca080625c7714b95ba7` |
...
| Ancla evidencia | Pending | `f4df818e30b64f7c74fbcfd54308a83a5d0648fb2018701bbe33b43d6fdaca33` |
| Auto Pending→InProgress | Confirmed | `e6df4aba0a3fb38179cd69b9eb817e3fdd06dbf0aa6ca67174cf065ac835e72f` |
| Observar (InProgress→Observed) | Pending | `5bf3fc45f50b9c91be9ebe3edde5565b6938dcffd7d76065f3d4832dfc5c4125` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `9b14ec76069b288397955f48fd40f5f2ae532c456be4c2e0d0b6860dcd39d6a2` |
| Ancla evidencia | Pending | `d4e4734b359498ae890865f40b3df84d7de5b22be792f547b51f3886a7c06d9d` |
...
| Certificar (InProgress→Completed) | Pending | `2f754af5c1b445d300ed81bc8a5ece986516518bdacbd17e072baf4cb19258a3` |
...
| Ancla evidencia | Pending | `b36aa79c33c85fbef36c25f270c8f7ac28521d75f3579b163ef01c8dd07b1aa2` |
| Auto Pending→InProgress | Confirmed | `ac7bd08fd8bee1bda6ddce08f89a6f0ff1dcfe98e1b14196777bb8f9df9d792c` |
| Observar (InProgress→Observed) | Pending | `ea8c78ef3244e8551af2e42db2a37fb40a4d14b091d6aa4797c0baf12385e03a` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `5fa9493e2208ed94696354d7ca7374acb97a2683bb496db46d91a96b39f422fc` |
| Ancla evidencia | Pending | `5cd74d393fd69e74d84eb156d66e4c40f7b489a9f003399a8d68a248918cc0c0` |
| Auto Pending→InProgress | Confirmed | `286586df2416fa21d83dd4aa17b40ab6dbb900ddd835adf6a3beada2ee9bebaf` |
| Observar (InProgress→Observed) | Pending | `80c693dfcb59abc77128c7a276750e27d1c0e80f89a6103f7178f02d58d5c543` |
| Reanudar (Observed→InProgress) | Confirmed | `255bfd5c4f577ec14031e920d4520e2f9656fb822fa07402f68343199d987f47` |
| Certificar (InProgress→Completed) | Pending | `0238a9ae5ae6bb5ac96595ca9a58eca215d52b2f0106bc155096a163c12b2d25` |

**Etapa 5 — Estructura planta baja**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `d2a43fe211837f40b1c273bd0196fbcb5dd67b329c7e0cee45883e210efff398` |
| Ancla evidencia | Pending | `c9f7e5817a409c2850ed8b082830de1542aabf2c382d46e9dfef48e7cd4f49a7` |
| Auto Pending→InProgress | Confirmed | `33f7deb053da97125c4ba61ab4b70af8ba00c3a9ecb9f556012275ddade0463a` |
| Observar (InProgress→Observed) | Pending | `ae376b86bd727974574c77a007bd07e0f69b24963bbc2ae778b2a2975efc278f` |
| Reanudar (Observed→InProgress) | Confirmed | `0069507b3c1aba88ad4852cf56265c2bf729b7857f35458d4d495062533c9455` |
| Certificar (InProgress→Completed) | Pending | `3031b0198fce05fe9b25efa5e3b28138ab06bc1baf00a00c86599cad527102bf` |

**Etapa 6 — Estructura niveles superiores**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `de5e195779b814a20b9a37938ab245f0c0e9b6b599683c9d564f6a69439ff235` |
| Ancla evidencia | Pending | `40fd381daecbc1239c73afd07ae0d8907c39a6e99d77dc063158b608f1ac50c7` |
| Auto Pending→InProgress | Confirmed | `c71d3593cacbd643498c26602fd557b4527c692f350c5ff35a76e95af04e3f0c` |
| Observar (InProgress→Observed) | Pending | `9d7696c5523f83fb3ebe2c8df7ba75c35eaaa92d53cc92c21468c68382b41dc9` |
| Reanudar (Observed→InProgress) | Pending | `a9cf646aca7a1b4d0fe16b31dbf2b8ed55f266689ea443d0b7c52efb31f70f6e` |
...
| Ancla evidencia | Pending | `158155667d6fb5fdc137407f3b998c75a8a14f0030c12f5457be5084c1614385` |
| Auto Pending→InProgress | Confirmed | `38b669b2f33c33d958b8f6f4ecafe2247045b05ada288aa8d1a20780bfbf84aa` |
| Observar (InProgress→Observed) | Pending | `19ebb84a3808df8417577c7dcef4292d246dfed0a4c363d1596526b1ed214c2d` |
| Reanudar (Observed→InProgress) | Pending | `102cc832b73b5aa3c95f7adc339d4c170a2ece3a7a8076df7090795909aa4814` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `735f7fab3ae469793fa1a4028f58c107f81849abd84ae60911d1f852222d1352` |
| Ancla evidencia | Pending | `4a37d7aaf2b6a51ea236e38bc7316a33d6aa944fc3f9b76497dfa93244cfeb4f` |
...
| Reanudar (Observed→InProgress) | Pending | `faa7d80e3b9538dff557fde2d43fac7f44d1e1674c690cfadab61b7b0cb2c261` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `c06642bba288567c32f141bd5da62ee82a52c12c23864473ba4120b6a7f1bd88` |
| Ancla evidencia | Pending | `ad7e1aefb94eb8a1b2867e33be8bf6d11fb79882bb9c8a0f2c008f912d9a1d0c` |
| Auto Pending→InProgress | Confirmed | `35f7ce36629175fff331a418e1efadd917a0984fe86e02ad71d35b2c0da2bdbd` |
| Observar (InProgress→Observed) | Pending | `477a5c8bb0b2958ab8ace93f497fa2d8375b08b5d4fddfba79889a289f7dcca4` |
...
| Ancla evidencia | Pending | `c34314d4276c030a43919d9d0f13b82edb3f36cb2e90cc19dfdde6e253f4a3ed` |
...
| Reanudar (Observed→InProgress) | Pending | `9ba0f810eb545ddf0060c9d092cdba0cbc563b53853c6509f9ec75f85b6da3ca` |
...
| Ancla evidencia | Pending | `439b10c4d6cde16501854011a7326c59b5315157c2b04708c1e23196398ede2e` |
| Auto Pending→InProgress | Confirmed | `2d8e33cb04a1e05b7f552e0c6e143adc149c7f55daad9923dec1417605e6c830` |
| Observar (InProgress→Observed) | Pending | `9c54abe7db22e85d1bcbcc0ca525142e41b104f4c1be8f711771377d4e0bc358` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `3144dac1045e54597f3df797b097b87a660c26477ba0595b91a68bc2be80d9ab` |
| Ancla evidencia | Pending | `f25230d359ca45f5aa0045412577cfc533f38bddc875ddc9ead7a0d131dc1ff6` |
| Auto Pending→InProgress | Confirmed | `2195d5cc0e277172f5e76cc2b2e4c2453f8913481d1192eb156e8ce2cd5727c1` |
| Observar (InProgress→Observed) | Pending | `9f29b951d7eb9d2b03328b341dcf970a5b51bf3632324007e10c032dfd4850be` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `bf850b52b5c2884952da8731c9599b39eb7602a60171e42c79695ed00a3391b3` |
| Ancla evidencia | Pending | `835b6b83fb1edf009fe97f9d7197b0f111acc77da3acccd560cdc01a72abc49f` |
...
| Certificar (InProgress→Completed) | Pending | `13de262947dc69cf7e8e63c02b373dbfcdb8436a1a0d3855935d2f0a64b499ba` |

**Etapa 4 — Cimentación**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `9b1bf8420d87c8ca697574ff3dfa2dd107c5ad5de54d4e6a07aa802f8a11abd1` |
| Ancla evidencia | Pending | `ac7625e149a06742f57d7cc5a8ab215647d96b880ce35e0ef39fb08662d12248` |
...
| Reanudar (Observed→InProgress) | Pending | `a91c57f134f0431df4d3065684a3c8d1ff7ae374d7fef6295bee6f4295bc9d4d` |
| Certificar (InProgress→Completed) | Pending | `f2e687527c8624bf006a5fd3708e98d4fa9c301d7ad2c943cb90d7907be93dfb` |

**Etapa 5 — Estructura planta baja**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `14a043d42ae6b6d0167edd3ffc1622dd057bd5aea14fe7cfc41ca5e6f8237e62` |
| Ancla evidencia | Pending | `237284c136ae30880d9dbad12ac912271f4d448356e9043ef5fb53a2dccc6769` |
| Auto Pending→InProgress | Pending | `b21dfeab948f0034759dba2d56b1f931ed63a6658535fde0128abb736774811e` |
...
| Certificar (InProgress→Completed) | Pending | `81bffe18c3f0c4463f9317a72d291df2980b661443b7bd46332673639d8baab5` |
...
| Ancla evidencia | Pending | `6d1892ea8a7029e9309df5d39ccc340204c99189cc8939eafc1fecf5de94aedc` |
...
| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `4d170440d8b8b916dc339bca9f0f9069d9cf34bf0df17ddc67a3d69b5e481498` |
| Ancla evidencia | Pending | `dfb63807a7a2a15c112ff2284df7be28a25703af692cb4f0b394f4ed650b1c2d` |
...
| Ancla evidencia | Pending | `36ebc69e2199fbe8da36d5d43499aebc3a84404e78da4dfff1e6371ed588e914` |
...
| Certificar (InProgress→Completed) | Pending | `fdf533c69cc1a0c2773f0e3938464c09f435e13dd4555d5bee8d64aed0e0001e` |

**Etapa 9 — Terminaciones**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `0dae3d845591855b666ffd1997124c4ca640dae617481af879a2acd9574b58fd` |
| Ancla evidencia | Pending | `3c08bf9dc899eb7c21b5d80f4f6c08d364a5a32b5ad5d6722ea6a30754a0d1eb` |
| Auto Pending→InProgress | Confirmed | `385c2e0192531aa42d5a93fcf89d0c810fab71e3e60342222c8bf9155d2da0a8` |
| Observar (InProgress→Observed) | Pending | `5f918bc177f99aadc44cde11f1abd2c5d25853ba0acab304051271acc1d7ac5c` |
| Reanudar (Observed→InProgress) | Confirmed | `44486b07ae68be5fdb7132541556bf84d45da5b8b1640dcdab7d3649304440c5` |
| Certificar (InProgress→Completed) | Pending | `792c435dd3827a92c1eaebba7330eb97ccdba9129178968b378d849ae04878ca` |

**Etapa 10 — Final de obra y subdivisión**

| Transición | Estado en base | TXID |
|---|---|---|
| Mint (declaración) | Pending | `f0bdc168eedc9f1c473c82964ccd79e65e9f4b326ced5eb01940afaa65dab563` |
| Ancla evidencia | Pending | `f36f1e5778943825ef55e2366097ad7e7c79fef40e48b65f913df9ac90777e4b` |
...
| Reanudar (Observed→InProgress) | Pending | `03007f19893539bafd50eb8ea4f9f15493192d5adb4f3a68650245cd1575f660` |
