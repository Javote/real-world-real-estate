...
| **3** | UI Implementation & Integration (flujos end-to-end en pre-prod, audit logs) | ✅ 53/53 superficies de M2-D5, audit log append-only y paginable. **Flujos end-to-end en pre-prod: cerrado el 2026-09-10** con la prueba de volumen (30/30 etapas `Completed`, 180/180 eventos on-chain `Confirmed`, las 4 aristas de la FSM ejercitadas en 3 proyectos reales) | ◐ **La mediana "reserva → creación de escrow < 12 min"** (Acceptance Criteria de este Output, ver criterio 9 del SOM en `specs/README.md`): el endpoint de telemetría ya existe (`GET /audit-logs/telemetry/reservation-to-escrow`, calcula la mediana sobre `OnChainEvent`), pero **nadie confirmó una reserva real en Preprod todavía** — la prueba de volumen ejercitó la FSM del stage, no el flujo de compra del investor. Falta correr ese flujo una vez y capturar la medición real |
| **4** | Testing & Security (unit/integration, static analysis, dependency scans, telemetría) | ✅ Tests + coverage corriendo en CI (`pnpm test:coverage`), Semgrep (`security-audit` + `owasp-top-ten`) automático en cada CI run, `specs/SECURITY-REVIEW-2026-09.md` con 3 P1 cerrados y 0 abiertos, Sentry + OTel→Grafana Cloud verificados con traces reales en producción | ◐ "Dependency scans" en plural — hoy `pnpm audit` es una corrida puntual documentada en el security review, no un paso que corra solo en cada CI run. Mejora chica, no bloqueante |
...
| Muestra real de reserva → escrow < 12 min (criterio 9) | Falta ejercitar el flujo, no falta código | Correr el flujo de compra del investor una vez en Preprod |
...
| Dependency scans continuos | Mejora chica | Sumar `pnpm audit` como paso de CI, no solo puntual |
...
Lo único que realmente falta *hacer* del lado técnico es ejercitar el flujo de
reserva → escrow del investor una vez contra Preprod real (alimenta el criterio 9). El resto son
tareas de evidencia/publicación que no tocan código.
