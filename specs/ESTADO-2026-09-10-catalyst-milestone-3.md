...
> (`specs/evidencia-m3/3-preprod/REPORTE-2026-09-10-prueba-de-volumen.md`).
...
| **4** | Testing & Security (unit/integration, static analysis, dependency scans, telemetría) | ✅ Tests + coverage corriendo en CI (`pnpm test:coverage`), Semgrep (`security-audit` + `owasp-top-ten`) automático en cada CI run, **`pnpm audit` en cada CI desde el 2026-09-11** (gate en crítico + reporte completo no bloqueante), `specs/evidencia-m3/4-seguridad/SECURITY-REVIEW-2026-09.md` con 3 P1 cerrados y 0 abiertos, Sentry + OTel→Grafana Cloud verificados con traces reales en producción | — |
| **5** | Pre-production Environment & Ops (seed, monitoring, URL pública, walkthrough, runbook) | ✅ URL pública viva (`propnexus-web.onrender.com` / `propnexus-api.onrender.com`), `specs/evidencia-m3/5-ops/RUNBOOK-deploy.md` completo, Sentry/Grafana/PostHog encendidos y con datos reales. **Screenshot formal de monitoring: cerrado el 2026-09-11** (`specs/evidencia-m3/5-ops/EVIDENCIA-2026-09-11-monitoring-screenshots.md`) | ⬜ **Video walkthrough** — nunca grabado (criterio 13 del SOM) |
